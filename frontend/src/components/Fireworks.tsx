...
function pickColors(group: string[], count = 5) {
  return [...group].sort(() => 0.5 - Math.random()).slice(0, count)
}

export default function Fireworks() {
  const [particles, setParticles] = useState<Particle[]>([])
  const explosionPoints = useMemo(() => generateExplosions(7, 18), [])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const colors = pickColors(COLOR_GROUPS[index % COLOR_GROUPS.length], 5)
